...
function OpenPopVehicleMenu()
	ESX.TriggerServerCallback('esx_vehicleshop:getCommercialVehicles', function(vehicles)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'commercial_vehicles', {
			...
			ESX.ShowNotification('Fordonet är utställd')
		end, function(data, menu)
			menu.close()
		end)
	end)
...
AddEventHandler('esx_vehicleshop:hasEnteredMarker', function(zone)
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealer' and ESX.PlayerData.job.grade_name == 'boss' then
		...
		-- CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	elseif zone == "ChangeClothes" and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealer' then
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			...
			if distance < Config.DrawDistance then
				...
				if v.Type ~= -1 then
					if k == "ResellVehicle" then
						DrawMarker(6, v.Pos - vector3(0.0, 0.0, 0.98), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 3.0, 3.0, 3.0, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 150, false, true, 2, false, nil, nil, false)
					else
						DrawMarker(6, v.Pos - vector3(0.0, 0.0, 0.98), 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 1.0, 1.0, 1.0, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 150, false, true, 2, false, nil, nil, false)
					end
				end
				...
				if distance < v.Radius then
					-- if k == "ResellVehicle" then
						-- if IsPedInAnyVehicle(PlayerPedId(), true) then
							-- Draw3D(v.Pos, v.Text, 255)
							-- isInMarker, currentZone = true, k
						-- else
							-- Draw3D(v.Pos, "Du måste sitta i ett fordon", 255)
						-- end
					-- else
						Draw3D(v.Pos, v.Text, 255)
						isInMarker, currentZone = true, k
...
OpenClothesMenu = function()
	...
	table.insert(elements, {label = "Garderob", value = "MyClothes"})
	table.insert(elements, {label = "Personliga outfit", value = "default_wear"})
	table.insert(elements, {label = "Arbetskläder", value = "work_wear"})
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if action == "MyClothes" then
				OpenWardrobe()
			elseif action == 'default_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
					TriggerEvent('skinchanger:loadSkin', skin)
				end)
				ESX.ShowNotification("Du tog på dig din Personliga outfit!")
			elseif action == 'work_wear' then
				if ESX.PlayerData.job.grade_name == 'boss' then
					SetPlayerUniform("boss_wear", PlayerPedId())
				else
					SetPlayerUniform(action, PlayerPedId())
				end
				ESX.ShowNotification('Du tog på dig dina Arbetskläder!')
			end
			menu.close()
		end,
	function(data, menu)
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "motel_main_dressing_menu", {
			...
			TriggerEvent("skinchanger:getSkin", function(skin)
                ESX.TriggerServerCallback("james_motels:getPlayerOutfit", function(clothes)
                    ...
                    ESX.ShowNotification("Du ändrade din outfit.")
                end, currentOutfit)
			end)
...
SetPlayerUniform = function(wear, playerPed)
    TriggerEvent('skinchanger:getSkin', function(skin)
        if skin.sex == 0 then
            if Config.Uniforms[wear].male then
                TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms[wear].male)
            else
                ESX.ShowNotification(_U('no_outfit'))
            end
        else
            if Config.Uniforms[wear].female then
                TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms[wear].female)
            else
                ESX.ShowNotification(_U('no_outfit'))
            end
        end
